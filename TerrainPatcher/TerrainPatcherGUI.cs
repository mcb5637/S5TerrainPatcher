...
{
    public partial class TerrainPatcherGUI : Form
    {
        private MainIO main;

        public TerrainPatcherGUI()
        {
            InitializeComponent();
        ...
        private void btnRead_Click(object sender, EventArgs e)
        {
            ...
            BtnSaveHeightMap.Enabled = true;
            BtnCreateTextureMap.Enabled = true;
        ...
        {
            SaveFileDialog sfd = new SaveFileDialog();
            sfd.Filter = "Bmp Dateien (*.bmp)|*.bmp|Alle Dateien (*.*)|*.*";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                HeightTextureMapIO ht = new HeightTextureMapIO(main.tdata);
                bool clamp = ht.WriteHeightMap(sfd.FileName);
                if (clamp)
                {
                    MessageBox.Show("Einige Höhen waren außerhalb der möglichen Werte (>=1521, <=6978), sie wurden zu den höchsten/niedrigsten Werten korigiert.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }

        private void BtnCreateTextureMap_Click(object sender, EventArgs e)
        {
            SaveFileDialog sfd = new SaveFileDialog();
            sfd.Filter = "Bmp Dateien (*.bmp)|*.bmp|Alle Dateien (*.*)|*.*";
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                HeightTextureMapIO ht = new HeightTextureMapIO(main.tdata);
                ht.WriteTextureMap(sfd.FileName);
            }
        }
    }
